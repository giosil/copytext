...
short startsWith(const char *text, const char *prefix);
short endsWith(const char *text, const char *suffix);
...
short endsWith(const char *text, const char *suffix)
{
	...
	for (int i = 0; i < len_p; i++) {
		if (suffix[len_p - 1 - i] != text[len_t - 1 - i]) return 0;
	}
	return 1;
}
...
short match(const char *text, const char *key, const char *value)
{
	short result = startsWith(value, text);
	if (!result) result = endsWith(value, text);
	return result;
}
...
void test()
{
	...
	printf("startsWith(\"%s\",\" h\") -> %d\n", text, startsWith(text, " h"));
	printf("startsWith(\"%s\",\"xx\") -> %d\n", text, startsWith(text, "xx"));
	printf("endsWith(\"%s\",\"0 \") -> %d\n", text, endsWith(text, "0 "));
	printf("endsWith(\"%s\",\"xx\") -> %d\n", text, endsWith(text, "xx"));
	printf("hashCode(\"%s\") -> %d\n", text, hashCode(text));
	...
	entry = searchEntry("polis", entries, &match);
	if (entry) {
		printf("searchEntry(\"polis\", entries, &match) -> entry.key=\"%s\", entry.value=\"%s\"\n", entry->key, entry->value);
	}
	else {
		printf("findEntry(\"polis\", entries, &match) -> NULL");
	}
}